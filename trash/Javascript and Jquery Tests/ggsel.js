...
driver.get('https://groups.google.com/forum/#!aboutgroup/thor-opensource-3d-printable-robotic-arm').then(function(){
  //assert(driver.wait(until.titleContains('Google'),1000)).then(function(){
  driver.wait(until.titleContains(' '),10000).then(function(){
      if(driver.title=='Grupos de Google'){
      console.log("YAAAAY");
      }
  //var elem = driver.findElement(By.tagName('div'));
  //console.log(elem);
      else{
         console.log("nope");
         var tits=driver.getTitle();
          console.log(tits);
      }
    driver.quit();
    });
